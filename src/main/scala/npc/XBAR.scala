...
class XBAR extends Module{
    ...
    switch(state) {
      ...
      is(sBusy) {
        when(io.xbar_in.bits.rresp === 1.U || io.xbar_in.bits.bresp === 1.U) {
          state := sIdle
        }
      }
    }
  ...
  io.xbar_out.bits.load_unsign := Mux(ifu_selected, io.ifu_axi_in.bits.load_unsign, io.isu_axi_in.bits.load_unsign)
  ...
  io.xbar_out.bits.arlen   := 1.U
  io.xbar_out.bits.awlen   := 1.U
  io.xbar_out.bits.awsize  := 4.U
