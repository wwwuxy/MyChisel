...
### scala.ScalaReflectionException: value fun3 is not a method
...
//inital
    ...
    when(csr === 0x300.U){      //mstatus
        csr_sel := 0.U
    }.elsewhen(csr === 0x305.U){    //mtvec
        csr_sel := 1.U
    }.elsewhen(csr === 0x341.U){    //mepc
        csr_sel := 2.U
    }.elsewhen(csr === 0x342.U){    //mcause
        csr_sel := 3.U
    }
...
//创建一个包含32个32位宽的序列，初始化为0，将该序列转换为一个矢量，再存储
    ...
    when(io.is_csr){
        when(io.is_ecall){
            ...
            CsrReg(3) := 11.U
            io.rd2 := CsrReg(1)
        }.elsewhen(fun3 === "b001".U){   //csrrw
            io.rd2 := CsrReg(csr_sel)
        }
    }.otherwise{
        io.rd1 := FileReg(rs1)
        io.rd2 := FileReg(rs2)
